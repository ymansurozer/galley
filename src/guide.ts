...
export function validateGuide(input: unknown): GuideValidation {
  ...
  if (typeof g.overview !== "string" || !g.overview.trim()) return { ok: false, reason: "guide.overview must be a non-empty string" };
  ...
  for (let i = 0; i < g.files.length; i++) {
    const raw = g.files[i];
    if (!raw || typeof raw !== "object") return { ok: false, reason: `guide.files[${i}] must be an object` };
    const f = raw as Record<string, unknown>;
    if (typeof f.path !== "string" || !f.path.trim()) return { ok: false, reason: `guide.files[${i}].path must be a non-empty string` };
    if (typeof f.summary !== "string" || !f.summary.trim()) return { ok: false, reason: `guide.files[${i}].summary must be a non-empty string` };
