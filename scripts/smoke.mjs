...
let desk, tmp;
const cleanup = () => { try { desk?.kill(); } catch { /**/ } try { tmp && rmSync(tmp, { recursive: true, force: true }); } catch { /**/ } };
...
try {
  ...
  git("init", "-q"); git("config", "user.email", "s@s.dev"); git("config", "user.name", "smoke");
  writeFileSync(path.join(tmp, "a.txt"), "one\ntwo\nthree\n");
  git("add", "-A"); git("commit", "-q", "-m", "init");
  writeFileSync(path.join(tmp, "a.txt"), "one\nCHANGED\nthree\n");

  desk = spawn("node", [CLI, "--repo", tmp, "--session", ID, "--port", String(PORT), "--no-open"], { stdio: "ignore" });
  ...
  await post("/api/ask", { path: "a.txt", lineNumber: 2, side: "additions", body: "why this change?" });
  ...
  const reply = JSON.parse(cli("comment", "--repo", tmp, "--session", ID, "--path", "a.txt", "--line", "2", "--side", "additions", "--role", "agent", "--body", "Because two became CHANGED."));
  ...
  for (const k of ["requestedChanges", "accepted", "rejected", "stagedFiles"]) assert.ok(Array.isArray(ev2.result[k]), `result.${k} is an array`);
